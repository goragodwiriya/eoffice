...
if (defined('ROOT_PATH')) {
    ...
    if (!$error) {
        ...
        if (!$error) {
            try {
                $db->query("DELETE FROM `".$_SESSION['prefix']."_user` WHERE `id` IN (1,2,3)");
                // default user
                $password_key = uniqid();
                $salt = uniqid();
                ...
                $sql .= "(2, 'approver', '$salt', '".sha1($password_key.'approver'.$salt)."', NULL, 0, '', 'ผู้อนุมัติ', '$today'),";
                $sql .= "(3, 'driver', '$salt', '".sha1($password_key.'driver'.$salt)."', NULL, 2, '', 'คนขับรถ', '$today');";
                $db->query($sql);
